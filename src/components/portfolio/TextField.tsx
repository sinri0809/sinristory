...
const TextField = forwardRef((props: Props, forwardedRef: Ref<HTMLInputElement>) => {
  ...
  return (
    <div
      ...
      tabIndex={0}
    >
      {categoryTitle !== "" && <h6 className="textfield-title">{categoryTitle}</h6>}
      <div 
        className="textfield-wrap"
        data-focus={props.value !== ""}
      >
        <input 
          // ref={forwardRef}
          type={props.type}
        ...
        <label htmlFor="textfield-input" className="textfield-placeholder">
          <span className="placeholder-text">
            {requiredMessage}
            {requiredMessage && props.required && <span className="required">&#42;</span>}
          </span>
        </label>
      </div>
